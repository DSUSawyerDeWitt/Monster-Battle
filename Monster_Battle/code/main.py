...
class Game:
    def __init__(self):
        ...
        self.running = True
        self.import_assets()
        background_audio = self.audio['music']
        background_audio.set_volume(.05)
        ...
        #data
        player_monster_list = ['Sparchu', 'Cleaf', 'Jacana', 'Gulfin', 'Pouch', 'Larvea', 'Charmadillo', 'Atrox', 'Draem']
    ...
    def get_input(self, state, data = None): #return values from UI to main
        if state == 'attack':
            ...
            heal_audio = self.audio['green']
            heal_audio.set_volume(0.1)
            heal_audio.play()
        elif state == 'switch':
            self.monster.kill()
            self.monster = data
    ...
    def apply_attack(self, target, attack):
        base_damage = ABILITIES_DATA[attack]['damage']
        attack_multiplier = ELEMENT_DATA[ABILITIES_DATA[attack]['element']][target.element]
        target.health -= base_damage * attack_multiplier
        AttackAnimationSprite(target, self.attack_frames[ABILITIES_DATA[attack]['animation']], self.all_sprites)
        attack_audio = self.audio[ABILITIES_DATA[attack]['animation']]
        attack_audio.set_volume(.1)
        attack_audio.play()
    ...
    def player_turn(self):
        ...
        if self.monster.health <= 0:
            ...
            if available_monsters:
                ...
                self.running = False
        #if defeated get next monster
        #if no monsters end the game
